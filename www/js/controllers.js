...
.controller('MapsCtrl', function($scope, Leaves) {
...
'</div>';
  ...
  */ // indra : end
  ...
  /* Google Maps for Tracker Integration : Start */
  //Time between marker refreshes
    ...
    var color, polyStore = {}, path = new google.maps.MVCArray(),
...
$scope.getMarkers = function() {
    ...
    //  console.dir(res);
        ...
        if(markerStore.hasOwnProperty(res[i].id)) {
         ...
         markerStore[res[i].id].setPosition(new google.maps.LatLng(res[i].position.lat,res[i].position.long));
          ...
          latLang = new google.maps.LatLng(res[i].position.lat,res[i].position.long)
            ...
            path = polyStore[res[i].id].getPath();
            path.push(latLang);   
          ...
          polyStore[res[i].id].setMap(map); // indra
     ...
     // window.setTimeout($scope.getMarkers(),INTERVAL);

  }
  /* Google Maps for Tracker Integration : End */
